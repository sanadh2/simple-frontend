...
import { bookmarksApiClient, type BookmarkFilters, type CreateBookmarkDTO, type UpdateBookmarkDTO } from '@/lib/bookmarks-api';
...
export function useCreateBookmark() {
  ...
  return useMutation({
    mutationFn: (data: CreateBookmarkDTO) => bookmarksApiClient.createBookmark(data),
    onSuccess: () => {
...
export function useJobStatus(jobId: string | null, enabled: boolean = true) {
  return useQuery({
    ...
    refetchInterval: (query) => {
      ...
      if (data?.state === 'completed' || (data?.state === 'failed' && !data?.canRetry)) {
        return false;
      }
      return 2000;
    },
