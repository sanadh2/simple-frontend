...
import { useDeleteBookmark, useRegenerateTags, useJobStatus, useRetryJob, bookmarkKeys } from '@/hooks/useBookmarks';
...
export function BookmarkCard({ bookmark }: { bookmark: Bookmark }) {
    ...
    if (!hasCheckedActiveJob && !currentJobId) {
      const checkActiveJob = async () => {
        try {
          ...
          const activeJob = await bookmarksApiClient.getActiveJobForBookmark(bookmark._id);
          if (activeJob && (activeJob.state === 'waiting' || activeJob.state === 'active')) {
  ...
  useEffect(() => {
    if (jobStatus.data?.state === 'completed') {
      ...
      queryClient.invalidateQueries({ queryKey: bookmarkKeys.detail(bookmark._id) });
  ...
  }, [jobStatus.data?.state, jobStatus.data?.canRetry, bookmark._id, queryClient, regenerateTags]);
  ...
  const handleRegenerateTags = () => {
    if (currentJobId && jobStatus.data?.state === 'failed' && jobStatus.data.canRetry) {
      retryJob.mutate(currentJobId, {
        onSuccess: () => {
          queryClient.invalidateQueries({ queryKey: ['bookmark-job', currentJobId] });
        },
      });
    } else {
  ...
  const getStatusBadge = () => {
    ...
    if (state === 'failed') {
      return (
        <Badge variant="outline" className="bg-red-50 text-red-700 border-red-200">
          ❌ Failed {jobStatus.data.attemptsMade > 0 ? `(Attempt ${jobStatus.data.attemptsMade}/${jobStatus.data.maxAttempts})` : ''}
        </Badge>
      );
    }
  ...
  const getButtonText = () => {
    ...
    if (state === 'failed') return jobStatus.data.canRetry ? '🔄 Retry' : '🔄 Regenerate Tags';
